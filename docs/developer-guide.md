...
All information regarding contributing to and progressing `wshjs-util` module can be found in this document.
...
```
git clone http://github.com/wiseplat/wshjs-util
npm install
```
...
Across all `wshjs-` repos, we enforce mandatory travis-ci and coveralls testing. We never `commit to master`. As a general policy, Coveralls.io results must always be above 95% for any `wshjs-` PR or commit. We want to ensure complete coverage across the board.

## Contributing

Please help better the ecosystem by submitting issues and pull requests. We need all the help we can get to build the absolute best linting standards and utilities. We follow the AirBNB linting standard. Please read more about contributing to `wshjs-util` in the `.github/CONTRIBUTING.md`.
